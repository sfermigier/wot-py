...
class ConnRefCounter(object):
    ...
    def increase(self, conn_id, ref_id):
        ...
        self._logr.debug(
            "Added ref {} to conn <{}> (current: {})".format(
    ...
    def decrease(self, conn_id, ref_id):
        ...
        if conn_id not in self._counter:
            self._logr.warning(
                "Attempted to decrease ref of unknown conn: {}".format(conn_id)
            )
            return
        ...
        try:
            ...
            self._logr.debug(
                "Removed ref {} from conn <{}> (current: {})".format(
                    ref_id, conn_id, len(self._counter[conn_id])
                )
            )
        except KeyError:
            self._logr.warning(
                "Attempted to remove unknown reference: {}".format(ref_id)
            )
